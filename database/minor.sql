...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 03, 2015 at 09:04 AM
...
(987, 'GS', 'Nesh', 'm', '1995-11-01', 1202431222, 'gsnesh@campuskiosk.in', 's', 'gs.nesh', 'password');
...
--
ALTER TABLE `admin`
  ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `mobile_number` (`mobile_number`), ADD UNIQUE KEY `username` (`username`);
...
ALTER TABLE `crs_teach_stud`
  ADD KEY `fk_stud` (`student_id`), ADD KEY `fk_teach` (`teacher_id`), ADD KEY `fk_crs` (`course_id`);
...
--
ALTER TABLE `student`
  ADD PRIMARY KEY (`enroll`), ADD UNIQUE KEY `mobile_no` (`mobile_no`);

--
-- Indexes for table `teacher`
--
ALTER TABLE `teacher`
  ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `mobile_no` (`mobile_no`), ADD UNIQUE KEY `email` (`email`), ADD UNIQUE KEY `username` (`username`);
